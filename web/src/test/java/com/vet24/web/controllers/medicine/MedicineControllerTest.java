...
// Были заполнены не все поля при изменении существующего Препарата (1 поле)
    ...
    public void getAllDosagesSuccess() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(URI + "/{medicineId}/dosage", 101L)
                ...
                .andExpect(jsonPath("$[0].dosageSize", Is.is(101)))
                .andExpect(jsonPath("$[0].dosageType", Is.is("PILLS")))

                .andExpect(jsonPath("$[1].id", Is.is(102)))
                .andExpect(jsonPath("$[1].dosageSize", Is.is(102)))
                .andExpect(jsonPath("$[1].dosageType", Is.is("PILLS")));
        assertThat(2).isEqualTo(getCountDosages(101L));
    }
    ...
    //400 - Дозировка другого лекарства
    @Test
    @DataSet(cleanBefore = true, value = {"/datasets/controllers/medicineController/user-entities.yml",
            "/datasets/controllers/medicineController/medicine.yml",
            "/datasets/controllers/medicineController/dosage.yml"})
