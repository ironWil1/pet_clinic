...
public class DoctorController {
    ...
    public DoctorController(PetService petService, DoctorService doctorService,
                            DiagnosisService diagnosisService, DiagnosisMapper diagnosisMapper, TreatmentService treatmentService, TreatmentMapper treatmentMapper, ProcedureMapper procedureMapper, ProcedureService procedureService) {
    ...
    public ResponseEntity<TreatmentDto> addTreatment(@PathVariable Long petId,
        ...
        Diagnosis diagnosis = pet.getDiagnoses().stream().filter(x -> x.getId().equals(diagnoseId)).findFirst().orElse(null);
        if(diagnosis == null){
            throw new NotFoundException("diagnose is aplied");
        }
        Set<Procedure> procedureSet = procedures.stream().map(x ->{
            Procedure procedure = procedureMapper.abstractNewProcedureDtoToProcedure(x);
            procedure.getMedicine().setId(null);
            return procedure;
        ...
        treatment.setProcedures(procedureSet);
        treatmentService.persist(treatment);
        return new ResponseEntity<>(treatmentMapper.treatmentToTreatmentDto(treatment),HttpStatus.OK);
    }
}
